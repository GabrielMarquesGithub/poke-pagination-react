...
export const PokeProvider = ({ children }: { children: ReactNode }) => {
  ...
  const handleSetPokeList = (generation: undefined | string) => {
    if (generation) {
      return setPokeList(pokeListSlice[generation as keyof typeof pokeListSlice]);
    }
    setPokeList(pokeListSlice.generationOne);
  };
  useEffect(() => {
    const AsyncFunc = async () => {
      const pokeInit = await axios.get<pokemonRequest>("https://pokeapi.co/api/v2/pokemon?limit=100000&offset=0");
      const pokeComplete = pokeInit.data.results.slice(pokeList.slice1, pokeList.slice2).map((poke, index) => {
        return {
          name: poke.name,
